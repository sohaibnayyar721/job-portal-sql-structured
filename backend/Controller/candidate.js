...
// Get Recently Applied Data Company
const candidateRecentlyApplied = async (req, res) => {
    let id = req.id
    try {

        let connection = await createConnection()
        let [findEmployer] = await connection.query(`
        SELECT jobs.jobTitle, jobs.date, jobs.location, jobs.minSalary,
                jobs.maxSalary, jobs.jobType, jobs.category,
                jobs.employerID, jobs.jobID,
                applicants.status,applicants.jobID,
                employer.companyLogo
        FROM applicants 
        JOIN jobs ON jobs.jobID = applicants.jobID
        JOIN employer ON employer.employerID = jobs.employerID
        WHERE applicants.candidateID = ? 
        `, [id])

        res.json({findEmployer: findEmployer})
...
module.exports = { candidateAuthentication, candidateDashboard, candidateLogin, candidateSignUp, candidateApplyJob, candidateRecentlyApplied }