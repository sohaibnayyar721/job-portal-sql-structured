...
function EmployerEditJob({ updateCurrentComponent, jobID }) {
    ...
    useEffect(() => {
        async function getSingleJob() {
            try {
                let postData = await fetch(`http://localhost:4500/employeer/edit/${jobID}`, {
                    method: "GET",
                    headers: {
                ...
                if (response.jobData) {
                    setJobTitle(response.jobData[0].jobTitle)
                    setJobDescription(response.jobData[0].jobDescription)
                    setCategory(response.jobData[0].category)
                    setMinSalary(response.jobData[0].minSalary)
                    setMaxSalary(response.jobData[0].maxSalary)
                    setExperience(response.jobData[0].Experience)
                    setType(response.jobData[0].jobType)
                    setPhoneNumber(response.jobData[0].phoneNumber)
                    setCountry(response.jobData[0].location)
                    setCity(response.jobData[0].city)
                    setCareerLevel(response.jobData[0].careerLevel)
                    setAddress(response.jobData[0].address)
                    setZipCode(response.jobData[0].zipCode)



                    // setApplicationDeadlineDate(response.jobData.applicationDeadlineDate)
    ...
    async function handleSave() {
        ...
        if (!isValid) {
            try {
                ...
                const getJob = await fetch('http://localhost:4500/employeer/edit/jobs', {
                    method: "PUT",
                    body: formData

                })
                const response = await getJob.json()
    ...
    async function handleEditJob(jobID) {
        ...
        try {
            ...
            console.log(response.jobData)
            if (response.jobData) {
                setJobTitle(response.jobData.jobTitle)
            }
        } catch (err) {
            console.log(err)
            alert("Server Cannot Respond")
        ...
        <div className='flex relative overflow-y-auto h-[88vh] flex-col gap-8 w-full  bg-[#eeeeee99] px-12 py-6'>
            ...
            <div className=' flex flex-col gap-4 w-full text-green-700 '>
                ...
                <div className='border-l-[5px] border-green-700 px-3 '>
                    <p className='text-black font-bold text-xl'>Edit Job</p>
                ...
                <div className='flex flex-col border-gray-300 border-b-[1px] gap-5 px-8 py-8'>
                    ...
                    <div className='flex flex-col gap-3 pb-8'>
                        <div className='flex flex-col gap-3 '>
                            ...
                            {/* <textarea className={`w-full h-36 text-top rounded p-3 outline-green-700 outline-[0.5px] bg-[#F5F5F5] ${JobDescValidator && 'border-[1px] border-red-700'}`} placeholder='Job Description'
...
{!zipCode || zipCodeTypeValidator ? <div><li>Zip Code</li></div> : null} */}
            ...
            <div className='flex gap-2'>
                <button
                    onClick={handleSave}
                    className='text-white hover:text-[#14A077] font-bold rounded hover:bg-white bg-[#14A077] w-44 px-2 py-3'>Save</button>

                <button
                    onClick={() => { updateCurrentComponent('myJobs') }}
                    className='text-white hover:text-[#14A077] font-bold rounded hover:bg-white bg-[#14A077] w-44 px-2 py-3'>Cancel</button>
            </div>
